...
public class KudosCommand extends Registerable {
	...
	public void handleView(Player sender, String playerName) {
		...
		if (offlinePlayer == null) {
			lib.getMessagesAPI().sendMessage("player-not-found", sender, new Replaceable("%player-name%", playerName));
			return;
		}
		UUID uuid = offlinePlayer.getUniqueId();
		KudoData playerKudoData = kudoManager.getKudoData(uuid);
		if (playerKudoData == null) {
			lib.getMessagesAPI().sendMessage("no-kudo-data", sender, new Replaceable("%player-name%", playerName));
			return;
		}
		lib.getMessagesAPI().sendMessage("player-kudo-count", sender,
